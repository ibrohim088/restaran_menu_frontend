<template>
  <div class="menu-item-card card h-100 shadow-sm">
    ...
    <div class="card-footer d-flex gap-2">
      <button @click="editItem(item._id)" class="btn btn-sm btn-outline-primary flex-grow-1">
        ✏️ Tahrirlash
      </button>
      <button @click="confirmDelete(item._id)" class="btn btn-sm btn-outline-danger">
        🗑️
      </button>
    </div>
  </div>
...
import { Flame, Utensils, CheckCircle2, XCircle } from 'lucide-vue-next'
...
const imageUrl = computed(() => {
  return normalizeMediaUrl(props.item.imageUrl || props.item.image || props.item.photo || '')
})
